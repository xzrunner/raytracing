...
ConvexPartTorus::ConvexPartTorus(double a, double b, double min_azimuth, double max_azimuth, double min_polar, double max_polar)
	: a(a)
    , b(b)
    , bbox(-a - b, a + b, -b, b, -a - b, a + b)
...
bool ConvexPartTorus::ShadowHit(const Ray& ray, float& tmin) const
{
	/*
	if (!bbox.Hit(ray))
		return (false);
...
Normal ConvexPartTorus::ComputeNormal(const Point3D& p) const
{
    Normal normal;
