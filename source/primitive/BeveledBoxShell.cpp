...
BeveledBoxShell::BeveledBoxShell(const Point3D& bottom, const Point3D& top, float bevel_radius)
    : p0(bottom)
    , p1(top)
    , br(bevel_radius)
{
	...
	//BeveledCylinder *ocd = std::make_shared<BeveledCylinder(p0.y,p1.y,br,br);
	...
	//BeveledCylinder *ocd1 = std::make_shared<BeveledCylinder(p0.x,p1.x,br,br);
	...
	//BeveledCylinder *ocd2 = std::make_shared<BeveledCylinder(p0.z,p1.z,br,br);
...
bool BeveledBoxShell::Hit(const Ray& ray, double& tmin, ShadeRec& sr) const
{
	if (bbox.Hit(ray))
	{
		double		t;
		...
		for (int j = 0; j < num_objects; j++)
			if (m_parts[j]->Hit(ray, t, sr) && (t < tmin)) {
				hit				= true;
				tmin 			= t;
				m_material	    = m_parts[j]->GetMaterial();	// lhs is GeometricObject::material_ptr
				normal			= sr.normal;
...
bool BeveledBoxShell::ShadowHit(const Ray& ray, float& tmin) const
{
	if (bbox.Hit(ray))
	{
